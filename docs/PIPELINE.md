...
| Plan-indbakke (Outlook → plan.json) (`plan-inbox.yml`) | cron `*/20` (reelt 1-3 t); manuelt | `data/plan.json`, Intervals-events, Outlook (via `apply_edit.py`) — læser forslag som kalender-events 1/1-2030 `FAF-INBOX <id>` og sletter dem ved succes | AZURE_*, INTERVALS_API_KEY, GITHUB_TOKEN |
