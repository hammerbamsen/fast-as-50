...
- Kald først en RETNING når `weightAvg7Change28d` er mindst 0,3 kg (op eller ned) eller `fatAvg7Change28d` mindst 0,5 procentpoint (tærsklerne står i `rules.directionMinKg`/`rules.directionMinPp`). Ellers: "stabil". Er felterne `null`, er serien for kort: skriv "stabil — for kort serie" og nævn hverken vinduet i dage eller feltnavne. `weightAvg7Change84d`/`fatAvg7Change84d` er det lange perspektiv (tolv uger) — brug det til at sætte 28d-retningen i sammenhæng, aldrig i stedet for.
