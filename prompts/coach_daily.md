...
1. `oneThing`: den ene handling der betyder mest i dag. Prioritér: readiness LOW / TSB under gulvet → aflast dagens pas; misset nøglepas → flyt eller drop, aldrig "indhent"; cut aktiv og `deltaVsPlan` over planen → én kost-handling fra principperne; ellers dagens pas præcist (navn, minutter; ERG/ikke ERG KUN når `disc` er `bike` — aldrig ved løb, styrke eller svøm). Brug pasets `note` til at beskrive indholdet (fx fast tempo på en probe) — gæt ikke intensitet ud fra navnet.
