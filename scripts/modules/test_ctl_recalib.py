...
def test_no_proposal_below_threshold_or_downward():
    assert cr.build_proposal(PLAN, 46.0, date(2026, 9, 14), "t") is None      # +1 < 3
    r = cr.build_proposal(PLAN, 40.0, date(2026, 9, 14), "t")
    assert r and "warning" in r and "changes" not in r                       # nedad = kun advarsel


def test_proposal_shape_and_protection():
    p = cr.build_proposal(PLAN, 50.3, date(2026, 9, 14), "t")
    pr.validate(p)
    ...
    sim, dates = pr.apply_changes(PLAN, p["changes"])
    assert dates == []
    assert _programs.week_meta(_programs.active_program(sim, "kennet", date(2026, 9, 14)), 3)["ctlTarget"] == 53
    assert _programs.week_meta(_programs.active_program(PLAN, "kennet", date(2026, 9, 14)), 3)["ctlTarget"] == 48
    t = pr.target_changes_for_data(PLAN, p["changes"])
